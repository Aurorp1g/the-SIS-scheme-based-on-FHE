#include "../include/PlainProcess.h"

void getShareByPlain(Picture& source, vector<SharePic>& shares, int k, int range_x, bool printAns) {
...
void getShareByPlain_VEC(Picture& source, vector<SharePic>& shares, int k, int range_x, bool printAns) {
  vector<ll> y = source.getSec();
  vector<int> X;
  int n = shares.size();
...
vector<int> recoveryDemo(vector<int>& Xtem, vector<int>& ytem, DecTools& tools, Params& parms, int index) {
  ...
  if (index < 64 * 64) {
    ...
    for (int i = 0; i < parms.getk(); i++) {
      ...
      m.push_back(tem);
      KT = KT * tem % parms.getP();
    }
    for (int i = 0; i < parms.getk(); i++) {
      long long temm = mypow(m[i], P - 2, P);
      long long temK = KT * temm % P;
      K.push_back(temK);
      invm.push_back(temm);
      A.push_back(temK * ytem[i]);
    }
    vector<int> D = getD(Xtem, k);
    vector<int> a;
    invKT = mypow(KT, P - 2, P);
    for (int i = 0; i < k; i++) {
      int tem = 0;
      for (int j = 0; j < k; j++) {
        tem += A[j] * D[j * k + k - 1 - i] % P;
      }
      tem = tem * invKT;
